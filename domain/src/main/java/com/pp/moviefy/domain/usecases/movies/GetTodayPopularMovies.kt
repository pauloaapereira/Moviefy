...
class GetTodayPopularMovies @Inject constructor(
    private val moviesRepository: IMoviesRepository,
    @IoDispatcher dispatcher: CoroutineDispatcher
) : FlowUseCase<GetTodayPopularMovies.Parameters, PagedResponse<Movie>>(dispatcher) {
    ...
    override fun execute(parameters: Parameters): Flow<Result<PagedResponse<Movie>>> {
        return flow {
            emit(
                moviesRepository.getTodayPopularMovies(
                    parameters.language,
                    parameters.page,
                    parameters.region
                )
